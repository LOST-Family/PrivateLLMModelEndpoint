...
/**
 * Queue Worker Client for Processing Remote Queue Fetches pending linking
 * requests from remote API, processes them with LM Studio, and submits results
 */
public class QueueWorkerClient {
	...
	public void processQueue() {
		try {
			...
			for (int i = 0; i < requests.size(); i++) {
				...
				System.out
						.println("Processing request " + (i + 1) + "/" + requests.size() + " (ID: " + request.id + ")");
	 ...
	 */
	private void submitResult(String requestId, boolean success, String playerTag, String errorMessage)
			throws IOException {
